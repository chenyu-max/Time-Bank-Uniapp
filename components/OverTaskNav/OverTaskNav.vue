	...
	<view class="task-nav" @click="goToTaskDetail">
		...
		<view class="info">
			...
			<view style="text-align: right; width: 50%;"><text style="font-weight: bold;">审核状态：</text>
				<text style="color: #19be6b" v-if="task.state === 'over'">已完成</text>
				<text style="color: #2d8cf0" v-if="task.state === 'ing'">待审核</text>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		computed: {
			agree() {
				return (100 * this.task.remarkAgree / 21).toFixed(0)
			},
			disAgree() {
				return (100 * this.task.remarkDisagree / 21).toFixed(0)
			}
		},
		methods: {
			goToTaskDetail() {
				if (this.task.projectResult === 'over') {
					uni.navigateTo({
						url: `/pages/overTaskDetail/overTaskDetail?projectId=${this.task.projectId}&situation=over`
					})
				} else {
					uni.navigateTo({
						url: `/pages/overTaskDetail/overTaskDetail?projectId=${this.task.projectId}`
					})
				}
			},
		},
